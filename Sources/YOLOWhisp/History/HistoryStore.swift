...
public final class HistoryStore: HistoryStoring {
    ...
    public func save(entry: HistoryEntry) throws {
        ...
        sqlite3_bind_text(stmt, 3, (entry.rawText as NSString).utf8String, -1, nil)
        if let pt = entry.processedText {
            sqlite3_bind_text(stmt, 4, (pt as NSString).utf8String, -1, nil)
        } else {
            sqlite3_bind_null(stmt, 4)
        }
        sqlite3_bind_double(stmt, 5, entry.duration)
        sqlite3_bind_text(stmt, 6, (entry.modelUsed as NSString).utf8String, -1, nil)
        if let ta = entry.targetApp {
            sqlite3_bind_text(stmt, 7, (ta as NSString).utf8String, -1, nil)
        } else {
